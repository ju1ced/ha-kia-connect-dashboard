...
class KiaDashboardCard extends HTMLElement {
  ...
  _renderBatteryTab(context = {}) {
    ...
    return `<main class="battery-detail" aria-label="Battery details">
      ...
      <div class="battery-detail-grid">
        ...
        <section class="battery-detail-card">${title("mdi:car-battery", "12V battery", "Auxiliary battery condition")}<div class="battery-detail-list"><div><span>Battery level</span><strong class="${battery12v.available ? "" : "unavailable"}">${this._safe(battery12v.text)}</strong></div><div><span>Fault state</span><strong>${statusText("battery_12v_fault", "Fault detected", "No fault")}</strong></div></div></section>
        ...
        <section class="battery-detail-card">${title("mdi:heart-pulse", "Battery health", "Capacity, energy, and pack condition")}<div class="battery-detail-list"><div><span>State of health</span><strong class="${health.available ? "good" : "unavailable"}">${this._safe(health.text)}</strong></div><div><span>Remaining energy</span><strong class="${remainingEnergy.available ? "" : "unavailable"}">${this._safe(remainingEnergy.text)}</strong></div><div><span>Nominal capacity</span><strong class="${capacity.available ? "" : "unavailable"}">${this._safe(capacity.text)}</strong></div><div><span>Pack voltage</span><strong class="${voltage.available ? "" : "unavailable"}">${this._safe(voltage.text)}</strong></div></div></section>
  ...
  _renderVehicleTab() {
    ...
    const windowControls = [
      ...
      if (!this._entity(openKey) && !this._entity(closeKey)) return "";
      return `<div><strong>${label}</strong><button data-entity-action="${openKey}" data-service="turn_on" data-confirm="Open ${label.toLowerCase()} window?" ${this._entity(openKey) ? "" : "disabled"}>Open</button><button data-entity-action="${closeKey}" data-service="turn_on" data-confirm="Close ${label.toLowerCase()} window?" ${this._entity(closeKey) ? "" : "disabled"}>Close</button></div>`;
    }).join("");    const vin = read("vin");
    ...
    return `<main class="vehicle-detail"><header class="vehicle-page-heading card"><div><span>Vehicle detail</span><h2>Ready-state check</h2><p>Review access points, exterior lights, and tire status before departure.</p></div><div class="vehicle-page-meta"><div class="vehicle-page-status ${summaryTone}"><ha-icon icon="${summaryIcon}"></ha-icon><strong>${summaryText}</strong></div>${this._entity("vin") ? `<button class="vehicle-vin${vin.available ? "" : " unavailable"}" data-info="vin"><ha-icon icon="mdi:identifier"></ha-icon><span><small>VIN</small><strong>${this._safe(vin.value)}</strong></span></button>` : ""}</div></header><section class="vehicle-detail-grid">
      ...
      <article class="vehicle-section card vehicle-openings">${heading}<div class="vehicle-opening-grid">${openings.map(([key, label, context]) => tile(key, key === "trunk" ? "mdi:car-back" : key === "hood" ? "mdi:car-cog" : "mdi:car-door", label, context, this._active(key), binaryLabel(key, "Open", "Closed"))).join("")}</div>${windowControls ? `<div class="vehicle-window-controls"><h3>Window controls</h3>${windowControls}</div>` : ""}</article>
  ...
  _vehicleTabStyles() {
    return `
      ...
      .vehicle-window-controls{margin-top:16px;padding-top:16px;border-top:1px solid var(--kia-line)}.vehicle-window-controls h3{margin:0 0 10px}.vehicle-window-controls>div{display:grid;grid-template-columns:1fr auto auto;align-items:center;gap:8px;margin-top:8px}.vehicle-window-controls button{min-height:34px;padding:0 12px;border:1px solid var(--kia-line);border-radius:8px;background:var(--kia-control)}.vehicle-window-controls button:disabled{cursor:not-allowed;opacity:.45}

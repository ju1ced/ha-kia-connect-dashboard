class KiaDashboardCard extends HTMLElement {
  ...
  _renderClimateTab() {
    ...
    const mappedState = (key) => this._entity(key) ? (this._active(key) ? "On" : "Off") : "Not mapped";
    return `<main class="climate-detail" aria-label="Climate details">
      ...
      <section class="climate-controls card"><div class="climate-heading"><ha-icon icon="mdi:remote"></ha-icon><div><span>Remote controls</span><h2>Prepare the cabin</h2></div></div><div class="climate-actions"><button data-action="start_climate"><ha-icon icon="mdi:fan"></ha-icon>Start Climate</button><button class="stop" data-action="stop_climate"><ha-icon icon="mdi:fan-off"></ha-icon>Stop Climate</button></div>${this._notice ? `<p class="notice">${this._safe(this._notice)}</p>` : ""}</section>
